...
public class QryEval {
    ...
    public static void main(String[] args) throws Exception {
        ...
        OUTPUT_PATH = parameters.get("trecEvalOutputPath");
        ...
        if (fb != null && fb.toLowerCase().equals("true")) {
            Map<String, ScoreList> scoreLists = null;
            ...
            model.fbOrigWeight = fbOrigWeight;

            if (parameters.get("fbInitialRankingFile") != null) {
                scoreLists = TrecReader.readPartialRes(parameters.get("fbInitialRankingFile"), fbDocs);
            } else {
...
//                ExpansionTermUtil.writeExpanQuery(parameters.get("queryFilePath"), fbExpansionQueryFile, tmpFile, fbOrigWeight);
    ...
    static void processQueryFile(String queryFilePath,
        ...
        try {
            ...
            //Top terms
            Map<String, List<ExpansionTermUtil.Entry>> topmTerms = model.topmTerms;

            //  Each pass of the loop processes one query.
            // Bufout persistant to file
            output = new BufferedWriter(new FileWriter(new File(OUTPUT_PATH)));
            ...
            while ((qLine = input.readLine()) != null) {
                ...
                //If fb is on
                if (model.fb) {
                    List<ExpansionTermUtil.Entry> tmp = topmTerms.get(qid);
                    StringBuilder sb = new StringBuilder().append("#wand (");
                    for (ExpansionTermUtil.Entry entry : tmp) {
                        sb.append(String.format("%.4f", entry.score) + " ").append(entry.term + " ");
                    }
                    sb.append(")");
                    query = "#wand ( " + model.fbOrigWeight + " #and (" + query + ") " + (1 - model.fbOrigWeight)
                            + " " + sb.toString() + " )";
                }
                ...
                if (r != null) {
                    output.write(printResults(qid, r));
                    System.out.println();
                }
            }
     ...
     * @throws IOException Error accessing the Lucene index.
     */
    static String printResults(String queryName, ScoreList result) throws IOException {
        result.sort();
        ...
        if (result.size() < 1) {
            ...
            for (int i = 0; i < result.size() && i < 100; i++) {
                System.out.println("\t" + i + ":  " + result.getDocid(i) + " " + Idx.getExternalDocid(result.getDocid(i)) + ", "
        ...
        if (result.size() < 1) {
            sb.append(queryName).append(" ").append("Q0 ").append("dummy ").append(1).append(" ").append(0).append(" ").append("run-1\n");
        } else {
            for (int i = 0; i < result.size() && i < 100; i++) {
                sb.append(queryName).append(" ").append("Q0 ").append(Idx.getExternalDocid(result.getDocid(i))).append(" ")
                        .append(i + 1).append(" ").append(result.getDocidScore(i)).append(" ").append("run-1").append("\n");
            }
        }
        return sb.toString();
    }
