...
public class QryParser {
    ...
    private static final Pattern WINDOW_PATTERN = Pattern.compile("#window/(.*)");
    ...
    private static Qry createOperator(String operatorName) {
        ...
        switch (operatorNameLowerCase) {
            ...
            default:
                // Here processing NEAR Operator Matching to find distance arg
                Matcher matcherNear = NEAR_PATTERN.matcher(operatorNameLowerCase);
                Matcher matcherWindow = WINDOW_PATTERN.matcher(operatorNameLowerCase);
                if (matcherNear.matches()) {
                    //If matches then extract argument
                    try {
                        int distance = Integer.parseInt(matcherNear.group(1));
                        operator = new QryIopNear(distance);
                    } catch (Exception e) {
                        syntaxError("Unknown query operator " + operatorName);
                    }
                    break;
                } else if (matcherWindow.matches()) {
                    //If matches then extract argument
                    try {
                        int distance = Integer.parseInt(matcherWindow.group(1));
    ...
    private static Qry parseString(String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            if (queryString.charAt(0) == '#') {    // Subquery
                ...
                //If matches digits then convert to the weight append to corresponding ops
                if (tmp.matches("\\d+[.]\\d+") && queryTree instanceof QrySopWeighted) {
                    QrySopWeighted qrySopWeighted = (QrySopWeighted) queryTree;
                    curWeight = Double.parseDouble(p.getPopped());
            ...
            for (int i = 0; i < qargs.length; i++) {
                ...
                if (queryTree instanceof QrySopWeighted) {
                    QrySopWeighted qrySopWeighted = (QrySopWeighted) queryTree;
                    qrySopWeighted.appendWeights(curWeight);
                }
            }
        }
